...
model_file = "url_Phishing_Model.pth"
scaler_file = "url_Phishing_Model.pkl"
...
# Add this line
admin_credentials = {'admin': '336699'}
...
def login():
    ...
    if username in admin_credentials and admin_credentials[username] == password:
        ...
        return redirect(url_for('index'))  # Redirect to prediction
    else:
        return "Invalid credentials", 401

# Use a different route for prediction
...
def index():
    ...
    if request.method == 'POST':
        ...
        input_data = {
                'card_rem': float(request.form.get('card_rem', 0)),
                'ratio_Rrem': float(request.form.get('ratio_Rrem', 0)),
                'ratio_Arem': float(request.form.get('ratio_Arem', 0)),
                'jaccard_RR': float(request.form.get('jaccard_RR', 0)),
                'jaccard_RA': float(request.form.get('jaccard_RA', 0)),
                'jaccard_AR': float(request.form.get('jaccard_AR', 0)),
                'jaccard_AA': float(request.form.get('jaccard_AA', 0)) 
        }
        print(input_data)
